        ...
        !awaitConditionForStatementOrTimeout(
          statement => {
            statement.error match {
              case Some(error)
                  if error == """{"message":"Fail to run query. Cause: No FileSystem for scheme \"s3\"","errorCode":"UNKNOWN_ERROR","exception.type":"org.apache.hadoop.fs.UnsupportedFileSystemException"}""" =>
              // Assertion passed
              case _ =>
